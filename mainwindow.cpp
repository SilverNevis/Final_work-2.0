...
{
    auto chatMessage = m_dbPtr->getChatMessages();
    QString chat;
    for(const auto &msg : chatMessage)
    {
...
void MainWindow::on_actionOpen_Server_triggered()
{
    ServerMainWindow *serverWindow = new ServerMainWindow(m_dbPtr, this);
    serverWindow->show();
}
